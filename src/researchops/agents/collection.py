"""Collector agent — gather and deduplicate research sources."""
...
from researchops.apps.research.prompts import COLLECTOR_QUERIES
from researchops.apps.research.schemas import PlanOutput, Source, SourceType
from researchops.core.context import RunContext
from researchops.core.state import AgentResult
from researchops.core.tools.registry import ToolPermissionError
...
class CollectorAgent(AgentBase):
    ...
    def execute(self, ctx: RunContext) -> AgentResult:
        ...
        if collect_round > 1:
            ctx.registry.clear_tools_cache("web_search", "fetch")
            ctx.registry.clear_tools_cache("arxiv_search", "semantic_scholar_search")
            strategy_level = max(strategy_level, 1)
            ctx.state.collect_strategy_level = strategy_level

        negative_terms = self._get_negative_terms(plan.topic, ctx)
        used_queries = self._load_used_queries(ctx)

        supervisor_queries, supervisor_neg_terms = self._load_supervisor_suggestions(ctx)
        if supervisor_neg_terms:
            for term in supervisor_neg_terms:
        ...
        if supervisor_queries:
            bucket_queries = supervisor_queries + bucket_queries
        ...
        if strategy == "demo":
            ...
            try:
                ...
                if not existing_sources:
                    sources.extend(self._collect_offline(ctx))
            # Fallback: if web search returned nothing useful, try academic + wiki
            new_from_web = len(sources) - len(existing_sources)
            if new_from_web == 0:
                ctx.trace.log(
                    stage="COLLECT", agent=self.name, action="web_fallback",
                    output_summary="Web search returned 0 sources, falling back to academic+wiki",
                )
                try:
                    s_acad, f_acad = self._collect_arxiv(
                        ctx, plan, gap_rqs, collect_round, strategy_level,
                        existing_hashes, negative_terms, used_queries, bucket_queries,
                    )
                    sources.extend(s_acad)
                    failures.extend(f_acad)
                except Exception:
                    pass
                if len(sources) < ctx.config.max_total_sources:
                    try:
                        wiki_sources = self._collect_wikipedia(
                            ctx, plan, collect_round, existing_hashes, gap_rqs,
                        )
            ...
            if len(sources) < ctx.config.max_total_sources:
                try:
                    ...
                    pass
            # Wikipedia as high-quality background knowledge supplement
            if len(sources) < ctx.config.max_total_sources:
                try:
                    wiki_sources = self._collect_wikipedia(
                        ctx, plan, collect_round, existing_hashes, gap_rqs,
        ...
        # Auto-escalate strategy when no new sources were found
        if new_count == 0:
            ctx.state.no_progress_streak += 1
            if ctx.state.no_progress_streak >= 2:
                ctx.state.collect_strategy_level = min(2, ctx.state.collect_strategy_level + 1)
        else:
    ...
    def _load_supervisor_suggestions(self, ctx: RunContext) -> tuple[list[str], list[str]]:
        """Read suggested queries and negative terms from the supervisor's last decision."""
        decision_path = ctx.run_dir / "last_decision.json"
        if not decision_path.exists():
            return [], []
        try:
            data = json.loads(decision_path.read_text(encoding="utf-8"))
            queries = data.get("suggested_queries", [])
            neg_terms = data.get("suggested_neg_terms", [])
            return queries, neg_terms
        except Exception:
            return [], []
    ...
    def _collect_arxiv(
        ...
        for rq in gap_rqs:
            ...
            for q_idx, query in enumerate(queries):
                ...
                # --- Fall back / supplement with Semantic Scholar ---
                if not arxiv_ok or len([r for r in results if "error" not in r]) < 2:
                    try:
                        s2_results = ctx.registry.invoke(
                            "semantic_scholar_search",
                            {"query": query, "max_results": max_results},
                            trace=ctx.trace,
                        )
                        if s2_results and not (len(s2_results) == 1 and "error" in s2_results[0]):
                            # Merge: deduplicate by title
                            existing_titles = {r.get("title", "").lower() for r in results if "error" not in r}
                            for p in s2_results:
                                if "error" not in p and p.get("title", "").lower() not in existing_titles:
                                    results.append(p)
                    except Exception:
                        pass
                ...
                for i, paper in enumerate(results[:papers_per_query]):
                    ...
                    new_srcs = self._save_paper_source(
                        paper, rq.rq_id, collect_round, q_idx, i,
                        query_id, downloads_dir, existing_hashes, ctx, failures,
                    )
    ...
    def _save_paper_source(
        ...
        """Persist a paper (arXiv or S2) as source files and return Source objects."""
        from_s2 = paper.get("source") == "semantic_scholar"
        s2_id = paper.get("s2_paper_id", "")
        arxiv_id = paper.get("arxiv_id", "")
        prefix = "s2" if (from_s2 and not arxiv_id) else "arxiv"
        source_id = f"src_{prefix}_{rq_id}_r{collect_round}_{q_idx}_{i}"
        ...
        if arxiv_id:
            ...
            url = paper.get("pdf_url", "") or f"https://arxiv.org/search/?query={paper.get('title', '')}"
            domain = "arxiv.org"
        ...
        if ctx.config.mode.value == "deep" and paper.get("pdf_url") and arxiv_id:
            try:
                ...
                if dl_result.get("status") == "success":
                    ...
                    if pdf_hash and pdf_hash not in existing_hashes:
                        pdf_source_id = f"src_arxiv_pdf_{rq_id}_r{collect_round}_{q_idx}_{i}"
                ...
                failures.append({
                    "rq_id": rq_id, "tool": "arxiv_download_pdf",
    ...
    def _collect_wikipedia(
        self, ctx: RunContext, plan: PlanOutput,
        collect_round: int, existing_hashes: set[str],
        gap_rqs: list | None = None,
    ) -> list[Source]:
        """Collect Wikipedia summaries as high-quality background knowledge."""
        sources: list[Source] = []
        downloads_dir = str(ctx.run_dir / "downloads")

        queries = [plan.topic]
        rq_pool = gap_rqs if gap_rqs else plan.research_questions[:3]
        for rq in rq_pool[:3]:
            short = rq.text.split("?")[0].split(",")[0].strip()
            if short and short != plan.topic:
        ...
        for q_idx, query in enumerate(queries[:3]):
            ...
            for i, article in enumerate(results):
                ...
                source_id = f"src_wiki_r{collect_round}_{q_idx}_{i}"
                local_path = Path(downloads_dir) / f"{source_id}_summary.txt"
                ...
                sources.append(Source(
                    ...
                    hash=content_hash,
                    source_type_detail="wikipedia",
                    collect_round=collect_round,
                    query_id=f"wiki_{q_idx}",
                ))
