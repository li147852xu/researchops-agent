...
class CollectorAgent(AgentBase):
    ...
    def execute(self, ctx: RunContext) -> AgentResult:
        ...
        existing_sources = self._load_existing_sources(ctx.run_dir)
    ...
    def _generate_queries(
        ...
        if ctx.reasoner.is_llm and strategy_level >= 1:
            try:
                ...
                prompt = (
                    f"Generate 3 diverse academic search queries for the research question: '{rq_text}' "
                    f"on the topic '{topic}'.{neg_hint} Each query should use different terminology "
                    f"and search angle. Return exactly 3 lines, one query per line. No numbering."
                )
    ...
    def _load_existing_sources(self, run_dir: Path) -> list[Source]:
