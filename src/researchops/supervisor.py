...
Serves as both a standalone policy advisor (legacy) and a graph router
when used as a node inside LangGraph.
"""
...
from researchops.models import Decision, PlanOutput, StateSnapshot
from researchops.prompts import SUPERVISOR_PLAN, parse_json_response
...
class Supervisor:
    ...
    def _compute_reason_codes(
        self, state: StateSnapshot, diag: dict, config: RunConfig,
    ) -> list[str]:
        codes: list[str] = []
    ...
    def _default_queries(self, topic: str, plan: PlanOutput | None) -> list[str]:
