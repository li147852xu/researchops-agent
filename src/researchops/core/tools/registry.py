...
class ToolRegistry:
    def __init__(self) -> None:
        ...
        self._session_cache_tool: dict[str, str] = {}   # cache_key → tool_name
    ...
    def invoke(
        ...
        if defn.cache_policy == "session":
            ...
            self._session_cache_tool[ck] = name
    ...
    def clear_cache(self) -> None:
        ...
        self._session_cache_tool.clear()

    def clear_tools_cache(self, *tool_names: str) -> None:
        """Remove session-cache entries belonging only to the specified tools."""
        to_remove = [ck for ck, tn in self._session_cache_tool.items() if tn in tool_names]
        for ck in to_remove:
            self._session_cache.pop(ck, None)
            self._session_cache_tool.pop(ck, None)
