...
def run(
    ...
    retrieval: RetrievalMode = typer.Option(RetrievalMode.BM25, "--retrieval", help="Retrieval mode: none, bm25, hybrid"),
    embedder: str = typer.Option("none", "--embedder", help="Embedder backend: none, openai_compat"),
    embedder_model: str = typer.Option("all-MiniLM-L6-v2", "--embedder-model", help="SentenceTransformer model for hybrid retrieval"),
    graph: bool = typer.Option(True, "--graph/--no-graph", help="Use LangGraph orchestrator (default: yes)"),
    judge: bool = typer.Option(False, "--judge", help="Enable LLM-as-Judge evaluation after run"),
    ...
    console.print(f"[bold]ResearchOps Agent v1.2.0[/] — run [cyan]{run_id}[/]")
    console.print(f"  Topic: {topic}")
    orch_label = "LangGraph" if graph else "sequential"
    console.print(
        f"  Mode: {mode.value} | Net: {net_enabled} | Sandbox: {sandbox.value} | "
        f"Sources: {sources.value} | Retrieval: {retrieval.value} | Orchestrator: {orch_label}"
    )
    ...
    if judge:
        console.print("  LLM-as-Judge: [green]enabled[/]")
    console.print()

    if graph:
        from researchops.orchestrator import GraphOrchestrator
        orch = GraphOrchestrator(config, run_dir)
    else:
        from researchops.orchestrator import Orchestrator
    ...
    eval_result = compute_eval(run_dir, config=config, enable_judge=judge, reasoner=reasoner)
    ...
    console.print(f"  Bucket coverage: {eval_result.bucket_coverage_rate:.1%}")
    console.print(f"  Relevance avg: {eval_result.relevance_avg:.2f}")
    ...
    if eval_result.judge_scores:
        console.print("\n[bold]LLM-as-Judge Scores:[/]")
        for dim, score in eval_result.judge_scores.items():
