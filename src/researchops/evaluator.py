...
from researchops.utils import load_plan, load_sources
...
def compute_eval(
    ...
    *,
    ...
    if enable_judge and reasoner is not None:
        try:
            from researchops.evaluator_llm import LLMJudge
...
def _sources_per_rq(sources: list[Source], run_dir: Path) -> dict[str, int]:
    ...
    for rq in plan.research_questions:
        ...
        count = sum(
            1 for s in sources
            if rq_id in s.source_id or rq_id in (s.query_id if hasattr(s, "query_id") else "")
        )
        result[rq_id] = count
    return result
