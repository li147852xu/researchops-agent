...
def test_supervisor_produces_decision(tmp_path: Path) -> None:
    """Supervisor produces Decision with reason_codes and action_plan."""
    ...
    assert decision.action_plan.get("queries") is not None
...
def test_qa_bucket_coverage_gate_triggers_rollback(tmp_path: Path) -> None:
    ...
    bucket_check = next((c for c in qa_report["checks"] if c["check"] == "bucket_coverage"), None)
...
def test_decisions_jsonl_written_and_valid(tmp_path: Path) -> None:
    """decisions.jsonl is written and contains valid JSONL."""
    from researchops.supervisor import Supervisor
    ...
    dec = Decision.model_validate(json.loads(lines[0]))
